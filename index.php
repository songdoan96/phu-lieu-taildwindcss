                ...
                <?php foreach ($items as $item) { ?>
                    <tr class="text-center text-sm odd:bg-gray-50 even:bg-gray-200 hover:bg-gray-300 dark:bg-gray-600  dark:hover:bg-gray-700 dark:text-white transition">
                        ...
                        <td class="border">
                            <?php
                            if (!$item->order_id) { ?>
                                <a class="bg-green-700 text-green-200 underline-offset-2 block py-2 px-1"
                            ...
                            <?php } else { ?>
                                <a class="no-underline flex justify-center text-green-600 dark:text-green-300 transition hover:scale-125"
                                   href="nhap-kho.php?id=<?= $item->item_id ?>">
                        ...
                        <td class="border">
                            <?php
                            if ($item->order_id) { ?>
                                <a class="bg-red-700 text-red-200 underline-offset-2 block py-2 px-1"
                                   href="xuat-kho.php?id=<?= $item->order_id ?>"><?= formatNumber($item->item_qty) ?></a>
                            <?php } else { ?>
                                <a class="no-underline flex justify-center text-red-600 dark:text-red-300 transition hover:scale-125"
                                   href="xuat-kho.php?id=<?= $item->item_id ?>">
                                    <svg fill="none" height="16" stroke="currentColor" stroke-linecap="round"
                                         stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" width="16">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
