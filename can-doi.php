                    ...
                    if ($styles) { ?>
                        <ul class=" space-y-3 p-4">
                            <?php foreach ($styles as $style) { ?>
                                <li>
                                    <a href='can-doi.php?customer=<?= $item->item_customer ?>&style=<?= $style->item_style ?>'
                                       class='flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-second-100 hover:bg-second-200 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white'><span
                                                class='flex-1 ml-3 whitespace-nowrap'>#<?= $style->item_style ?></span></a>
                                </li>
                            <?php } ?>
                        </ul>
                    <?php }
...
if (isset($_GET['style'], $_GET['customer'], $_GET['type'])) { ?>
    <div class="container mx-auto p-4">
        <form id="form-add-plan" method="POST" action="<?= $_SERVER['PHP_SELF'] ?>"
            ...
            <div class="flex flex-wrap items-center mb-2">
                <label for="customer"
                    ...
                    hàng:</label>
                <input readonly type="text" id="customer" name="customer" value="<?= get('customer') ?>"
            ...
            <div class="flex flex-wrap items-center mb-2">
                <label for="style" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Mã
                    hàng:</label>
                <input readonly type="text" id="style" name="style" value="<?= get('style') ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="type" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Mã
                    hàng:</label>
                <input readonly type="text" id="type" name="type" value="<?= get('type') ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="color" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Màu:</label>
                <input readonly type="text" id="color" name="color" value="<?= get('color') ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="size" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Màu:</label>
                <input readonly type="text" id="size" name="size" value="<?= get('size') ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="params"
                       class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Màu:</label>
                <input readonly type="text" id="params" name="params" value="<?= get('params') ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="plan" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">KHSX:</label>
                <input required type="number" step="0.01" id="plan" name="plan"
                       value="<?= !empty("plan") ? get('plan') : "" ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            <div class="flex flex-wrap items-center mb-2">
                <label for="norm" class="w-full mb-2 md:mb-0 md:w-1/4 block font-medium text-gray-900 dark:text-black">Định
                    mức:</label>
                <input required type="number" step="0.01" id="norm" name="norm"
                       value="<?= !empty("norm") ? get('norm') : "" ?>"
                       class="w-full md:w-3/4 rounded bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-primary-600 focus:border-primary-600 block p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500">
            </div>
            ...
            <div class="flex gap-2 justify-end mt-4">
                <a href="index.php"
                   class="flex  px-5 py-2.5 text-sm font-medium text-center text-white bg-red-700 rounded-lg focus:ring-4 focus:ring-red-200 dark:focus:ring-red-900 hover:bg-red-800">
                    Hủy
                </a>
                <button type="submit"
        ...
        <div class="overflow-x-auto md:overflow-x-hidden shadow-md">
            ...
            <table class="w-full border-collapse table-auto text-xs">
                ...
                $items = DB::table('items')
                    ...
                    ->fetchAll();
                foreach ($items as $item) { ?>
                    <tr itemId="<?= $item->item_id ?>"
                        class="text-center text-sm odd:bg-gray-50 even:bg-gray-200 hover:bg-gray-300 dark:bg-gray-600  dark:hover:bg-gray-700 dark:text-white transition">

                        <td class="border px-1 py-2"><?= $item->item_customer ?></td>
                        <td class="border px-1 py-2"><a class="underline"
                                                        href="items.php?ma-hang=<?= $item->item_style ?>"><?= $item->item_style ?></a>
                        </td>
                        <td class="border px-1 py-2"><a class="underline"
                                                        href="items.php?ma-hang=<?= $item->item_style ?>&type=<?= $item->item_type ?>&color=<?= $item->item_color ?>&size=<?= $item->item_size ?>&params=<?= $item->item_params ?>"><?= $item->item_type ?></a>
                        </td>
                        ...
                        <td class="border px-1 py-2">
                            ...
                            if ($item->item_plan) { ?>
                                <a class="underline"
                                   href="can-doi.php?customer=<?= $item->item_customer ?>&style=<?= $item->item_style ?>&type=<?= $item->item_type ?>&color=<?= $item->item_color ?>&size=<?= $item->item_size ?>&params=<?= $item->item_params ?>&plan=<?= $item->item_plan ?>&norm=<?= $item->item_norm ?>"><?= $item->item_plan ?></a>
                            <?php } else { ?>
                                <a class="no-underline flex justify-center transform hover:text-blue-500 transition hover:scale-110"
                                   href="can-doi.php?customer=<?= $item->item_customer ?>&style=<?= $item->item_style ?>&type=<?= $item->item_type ?>&color=<?= $item->item_color ?>&size=<?= $item->item_size ?>&params=<?= $item->item_params ?>&plan=<?= $item->item_plan ?>&norm=<?= $item->item_norm ?>">
